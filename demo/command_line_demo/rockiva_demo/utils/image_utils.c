...
static int ReadImageStb(const char* path, RockIvaImage* image)
{
    ...
    image->info.height = h;
    if (c == 4) {
        image->info.format = ROCKIVA_IMAGE_FORMAT_RGBA8888;
    } else if (c == 1) {
        image->info.format = ROCKIVA_IMAGE_FORMAT_GRAY8;
    } else {
        image->info.format = ROCKIVA_IMAGE_FORMAT_RGB888;
    }
    return 0;
...
int EncodeImageJpeg(RockIvaImage* src_image, RockIvaImage* dst_image, int quality)
{
    int ret;
    int jpegSubsamp = TJSAMP_422;
    unsigned char* jpegBuf = NULL;
    ...
    const unsigned char* data = src_image->dataAddr;
    int width = src_image->info.width;
    int height = src_image->info.height;
    int pixelFormat = TJPF_RGB;

	tjhandle handle = tjInitCompress();

    if (src_image->info.format == ROCKIVA_IMAGE_FORMAT_RGB888) {
        ret = tjCompress2(handle, data, width, 0, height, pixelFormat, &jpegBuf, &jpegSize, jpegSubsamp, quality, flags);
    } else {
	...
	// printf("ret=%d jpegBuf=%p jpegSize=%d\n", ret, jpegBuf, jpegSize);
    if (jpegBuf != NULL && jpegSize > 0) {
        dst_image->channelId = src_image->channelId;
        dst_image->frameId = src_image->frameId;
        dst_image->info = src_image->info;
        dst_image->info.format = ROCKIVA_IMAGE_FORMAT_JPEG;
        dst_image->size = jpegSize;
        dst_image->dataAddr = malloc(jpegSize);
        memcpy(dst_image->dataAddr, jpegBuf, jpegSize);
        tjFree(jpegBuf);
    }
    tjDestroy(handle);
...
int GetImagePathList(const char* dir, char (*image_path_list)[IMAGE_PATH_MAX], int max_num)
{
    ...
    for (int i = 0; i < n; i++) {
        ...
        snprintf(image_path_list[index], IMAGE_PATH_MAX, "%s/%s", dir, namelist[i]->d_name);
...
void DrawRectangleRGB(unsigned char* pixels, int w, int h, int rx, int ry, int rw, int rh, const unsigned char color[3],
                      int thickness)
{
    int stride = w * 3;
...
void DrawLineRGB(unsigned char* pixels, int w, int h, int x0, int y0, int x1, int y1, const unsigned char color[3], int thickness)
{
    int stride = w * 3;
...
int ResizeBilinearGray(char* src_pixels, int w, int h, char* dst_pixels, int w2, int h2) {
...
void DrawTextRGB(unsigned char* pixels, int w, int h, const char* text, int x, int y, int fontpixelsize, const unsigned char color[3])
{
    int stride = w * 3;
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (isprint(ch) != 0)
        {
            ...
            ResizeBilinearGray(font_bitmap, 20, 40, resized_font_bitmap, fontpixelsize, fontpixelsize * 2);
            ...
            for (int j = cursor_y; j < cursor_y + fontpixelsize * 2; j++)
            {
                ...
                for (int k = cursor_x; k < cursor_x + fontpixelsize; k++)
                {
                    ...
                    p[k * 3 + 0] = (p[k * 3 + 0] * (255 - alpha) + color[0] * alpha) / 255;
                    p[k * 3 + 1] = (p[k * 3 + 1] * (255 - alpha) + color[1] * alpha) / 255;
                    p[k * 3 + 2] = (p[k * 3 + 2] * (255 - alpha) + color[2] * alpha) / 255;
                }
            }
...
int DrawLine(RockIvaImage *image, RockIvaPoint start, RockIvaPoint end) {
    ...
    DrawLineRGB(image->dataAddr, image->info.width, image->info.height, pt1x, pt1y, pt2x, pt2y, COLOR_BLUE, 2);
    return 0;
}
