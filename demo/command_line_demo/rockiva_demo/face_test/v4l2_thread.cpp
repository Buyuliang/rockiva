...
void V4l2Thread::v4l2_get_one_frame(FrameBuffer *framebuf){
    ...
    process((char *)framebuf->buf, framebuf->length);
}

int V4l2Thread::process(char *buf, int len) {
    ...
    iva_ctx.imgBufManager =
        CreateImageBufferManagerPreAlloc(ALLOC_IMAGE_BUFFER_NUM, 640, 480, ROCKIVA_IMAGE_FORMAT_RGB888, ALLOC_IMAGE_BUFFER_TYPE);
